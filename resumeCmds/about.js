...
exports.invoke = function (res, options, shell) {
    ...
    res.log();
    res.type('I am proud of Shotgun by itself, but if the developer intends to use it in the browser as you are seeing now, there is a fair amount of legwork to get that running.');
    res.type('To make this easier I created a second module called "shotgun-client". This module uses the popular socket.io module in the background to communicate between the browser and the server in realtime.');
    res.type('My shotgun-client module requires one line of code on the server and a single reference in the client in order to use shotgun from within the browser.');
    res.log();
    res.type('While this solves the issue of sending user input across the wire to shotgun running on the server and vice versa, there is still the issue of creating all the elments in the browser for the user to supply input, the response information to be displayed, etc.');
    res.type('Fortunately shotgun-client also comes with a JQuery plugin. This plugin only requires one line of code in the browser to create the interface you are currently using.');
    res.log();
    res.type('The shotgun-client script reference includes a reference to another JQuery plugin I wrote called "jquery.coolType". This plugin allows text to be typed out character by character in the browser, giving the interface a hollywood hi-tech feel. CoolType includes a lot of options as well as some hooks that allow you to tap into its functionality, this is what allows us to play a sound when the characters are typing and stop the sound when they are finished. Sounds are played using the SoundManager2 JavaScript library.');
    res.log();
